...
Auth subcommand handlers: `commands/login.ts` (prompt for and store the API key) and `commands/logout.ts` (remove it). Arg parsing and command dispatch live in `index.ts` (`node:util` `parseArgs`), not a CLI framework.
